...
export default async function publicApiRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/api/v1/pricing", async () => ({
    ...
    tools: {
      free: {
        count: 11,
        list: ["optimize_bandit", "optimize_contextual", "optimize_evolve", "solve_schedule", "score_convergence", "score_calibration", "predict_bayesian", "predict_ensemble", "plan_pathfind", "simulate_montecarlo", "simulate_scenario"],
    ...
    tiers: {
      ...
      pay_per_call: {
        price: "$0.005/call",
        calls_per_day: 1000,
        tools: "All 17 tools (11 free + 6 premium)",
